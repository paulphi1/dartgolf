...
# Course mode: one pairing per hole • Tee sheet • Simulation clock
# Manual mode: your group frees the hole immediately and waits BETWEEN holes for your score.
# Stable user detection (user_index). Recovery helpers. Pause auto on your turn.
# Round hand-off fixed (R1→R2→R3→R4). Includes crash-safety to surface tracebacks.
...
def end_round_if_ready(state):
    """Return True when a round has ended and we advanced round_num.
       Sets flags so the next tick can kick off Round N cleanly."""
...
def tick_clock(state, dt):
    ...
    if finished and state["round_num"] <= NUM_ROUNDS:
        ...
        # align wall clock so auto loop doesn't overrun immediately
...
def init_state():
    ...
    # round hand-off flags
    s.prev_auto_running=False
    s.just_started_round=False
    s.round_started_at_sim=0.0
...
if not bots_source:
    upl=st.sidebar.file_uploader("Upload dgplayers.txt", type=["txt"])
...
with c1:
    if st.button("Load players"):
        if not bots_source: st.sidebar.error("Please provide dgplayers.txt.")
        else:
            try:
            ...
            except Exception as e: st.sidebar.error(f"Failed to load bots: {e}")
with c2:
    if st.button("Start game"):
        if not st.session_state.player_file_loaded: st.sidebar.error("Load players first.")
        else: start_new_game(st.session_state.bots_df)
...
with left:
    ...
    # Round just started? announce + optionally resume auto
    ...
    # Rebuild card if awaiting input but lost the card
    ...
    # Pause auto while you enter score (no jumping)
...
if (
    ...
    and st.session_state.pending_turn is None  # don't tick while entering a score
